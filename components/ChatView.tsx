...
import { Send, X, Check, CheckCheck, Mic, Play, Pause, PhoneMissed, Clock, Calendar, Edit2, Phone, Video, UserMinus } from 'lucide-react';
...
export const ChatView: React.FC<ChatViewProps> = ({ 
  ...
  const startRecording = () => {
      ...
      recordingIntervalRef.current = setInterval(() => {
          ...
          const limit = recordingMode === 'AUDIO' ? 60 : 15;
          if (duration >= limit) {
              stopRecording(true); 
          }
      }, 100);
  ...
  const stopRecording = (shouldSend: boolean) => {
      ...
      if (shouldSend) {
          ...
          const newMessage: Message = {
              ...
              audioUrl: recordingMode === 'AUDIO' ? 'mock_audio.mp3' : undefined,
              videoUrl: recordingMode === 'VIDEO' ? 'mock_video.mp4' : undefined,
          };
  ...
  // --- Enhanced Audio Bubble ---
  const AudioBubble = ({ msg, isMe }: { msg: Message, isMe: boolean }) => {
      ...
      useEffect(() => {
          if (isPlaying) {
              intervalRef.current = setInterval(() => {
                  setCurrentTime(prev => {
                      const next = prev + (0.1 * speed); // 100ms * speed factor
      ...
      return (
          <div className={`flex flex-col gap-2 min-w-[200px] ${isMe ? 'items-end' : 'items-start'}`}>
              <div className="flex items-center gap-3 w-full">
                              ...
                              return (
                                  <div 
                                    ...
                                    className={`w-1 rounded-full transition-all duration-200 ${
                                        isPlayed 
                                        ? (isMe ? (currentTheme.isDark ? 'bg-white' : 'bg-black') : 'bg-white')
                                        : (isMe ? (currentTheme.isDark ? 'bg-white/30' : 'bg-black/20') : 'bg-white/40')
                                    } ${isPlaying && i === Math.floor(progressPercent * 20) ? 'animate-pulse scale-y-125' : ''}`}
                                    style={{ 
                  ...
                  <button 
                    ...
                    className={`px-2 py-1 rounded-md text-[10px] font-bold transition-colors ${
                        isMe 
                        ? (currentTheme.isDark ? 'bg-white/20 hover:bg-white/30 text-white' : 'bg-black/10 hover:bg-black/20 text-black')
              ...
              {/* Timing Display */}
              <div className={`flex justify-between w-full px-1 text-[9px] font-mono font-medium ${isMe ? (currentTheme.isDark ? 'text-white/70' : 'text-slate-600') : 'text-white/80'}`}>
  ...
  const VideoBubble = ({ msg }: { msg: Message }) => {
      const mockThumb = match.profile.images[0]; 

      return (
          <div className="relative rounded-xl overflow-hidden aspect-[3/4] max-w-[160px] cursor-pointer group" onClick={() => setViewingImage(mockThumb)}>
               <img src={mockThumb} className="w-full h-full object-cover opacity-80" alt="Video message" />
