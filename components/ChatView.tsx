...
export const ChatView: React.FC<ChatViewProps> = ({
    ...
    const [entryDissolvePhase, setEntryDissolvePhase] = useState<'hidden' | 'visible' | 'fading'>('hidden');
    ...
    const scheduledReplyTimeoutRef = useRef<ReturnType<typeof setTimeout> | null>(null);
    ...
    useEffect(() => {
        ...
        const rafId = window.requestAnimationFrame(() => {
            setEntryDissolvePhase('fading');
        });
        const doneId = window.setTimeout(() => {
            setEntryDissolvePhase('hidden');
            onEntryDissolveDone?.();
        }, 260);
    ...
    // Scheduled Messages Checker (Runs at the next scheduled time; no polling interval)
    useEffect(() => {
        ...
        const now = getNowMs();
        const hasDueMessages = messages.some(
            (m) => m.isScheduled && typeof m.scheduledFor === 'number' && m.scheduledFor <= now
        );

        if (hasDueMessages) {
            // Defer to avoid setState directly in the effect body (React lint rule).
            schedulerTimeoutRef.current = setTimeout(() => {
                schedulerTimeoutRef.current = null;
                const fireNow = getNowMs();
                const stillHasDue = messagesRef.current.some(
                    (m) => m.isScheduled && typeof m.scheduledFor === 'number' && m.scheduledFor <= fireNow
                );
                if (!stillHasDue) return;
        ...
        const nextScheduledAt = messages
            .filter((m) => m.isScheduled && typeof m.scheduledFor === 'number' && m.scheduledFor > now)
            .reduce<number | null>((min, m) => {
                const scheduledFor = m.scheduledFor as number;
                if (min === null) return scheduledFor;
                return scheduledFor < min ? scheduledFor : min;
            }, null);

        if (nextScheduledAt !== null) {
            const delayMs = Math.max(0, nextScheduledAt - now);
            const safeDelayMs = Math.min(delayMs, 2_147_483_647); // setTimeout max (~24.8 days)
    ...
    useEffect(() => {
        ...
        const resetId = setTimeout(() => setCallDurationSeconds(0), 0);
        return () => {
    ...
    const handleScheduleConfirm = () => {
        ...
        const scheduleNow = getNowMs();

        const scheduledTime = new Date(scheduledDate).getTime();

        const scheduledMessage: Message = {
            id: `scheduled-${scheduleNow}`,
            text: inputText.trim(),
            senderId: 'me',
            timestamp: scheduleNow, // Created now
            status: 'scheduled',
            isScheduled: true,
    ...
    const entryDissolveBackground = useMemo((): string => {
        if (isPremium) {
            return 'radial-gradient(circle at 50% 42%, rgba(251,191,36,0.55), rgba(245,158,11,0.32) 32%, rgba(15,23,42,0.94) 76%), linear-gradient(130deg, rgba(251,191,36,0.26), rgba(15,23,42,0.96))';
        }
        if (currentTheme.isDark) {
            return 'radial-gradient(circle at 50% 42%, rgba(251,191,36,0.35), rgba(245,158,11,0.18) 30%, rgba(15,23,42,0.9) 75%), linear-gradient(130deg, rgba(245,158,11,0.18), rgba(15,23,42,0.94))';
        }
        return 'radial-gradient(circle at 50% 42%, rgba(251,191,36,0.26), rgba(245,158,11,0.12) 32%, rgba(248,250,252,0.88) 78%), linear-gradient(130deg, rgba(245,158,11,0.16), rgba(241,245,249,0.9))';
    }, [currentTheme.isDark, isPremium]);
        ...
        <div className={`flex flex-col h-full animate-fade-in relative ${currentTheme.backgroundColorClass}`} style={{ backgroundImage: currentTheme.backgroundImage, backgroundSize: 'cover', backgroundPosition: 'center' }}>
            {entryDissolvePhase !== 'hidden' && (
                <div
                    ...
                    style={{ background: entryDissolveBackground }}
                />
            )}
                ...
                {isMediaMenuOpen && !isRecording && !isScheduleMenuOpen && (
                    <div className={`absolute bottom-20 right-4 border rounded-xl shadow-2xl p-2 flex flex-col gap-2 animate-fade-in z-30 mb-2 ${currentTheme.isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}>
                        <button
                            ...
                            className={`flex items-center gap-3 px-4 py-2 rounded-lg text-sm font-bold transition-colors ${recordingMode === 'AUDIO' ? 'bg-gold-500 text-white' : `${currentTheme.isDark ? 'hover:bg-slate-700 text-slate-300' : 'hover:bg-slate-100 text-slate-600'}`
                                }`}
                        >
                        ...
                        <button
                            ...
                            className={`flex items-center gap-3 px-4 py-2 rounded-lg text-sm font-bold transition-colors ${recordingMode === 'VIDEO' ? 'bg-gold-500 text-white' : `${currentTheme.isDark ? 'hover:bg-slate-700 text-slate-300' : 'hover:bg-slate-100 text-slate-600'}`
                                }`}
                        >
            ...
            {isDatePickerOpen && (
                <div className="fixed inset-0 z-[100] bg-slate-950/90 backdrop-blur-sm flex items-center justify-center p-6 animate-fade-in" role="dialog" aria-modal="true" aria-label="Schedule message">
                    <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 max-w-sm w-full shadow-2xl">
                        ...
                        <div className="space-y-4 mb-6">
                            <div>
                                <label htmlFor="schedule-datetime" className="text-xs font-bold text-slate-500 uppercase mb-2 block">Pick Date & Time</label>
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        min={new Date(nowMs).toISOString().slice(0, 16)}
                                        max={new Date(nowMs + 7 * 24 * 60 * 60 * 1000).toISOString().slice(0, 16)} // 7 days max
            ...
            {viewingImage && (
                <div className="fixed inset-0 z-[100] bg-black/95 flex items-center justify-center animate-fade-in p-4" role="dialog" aria-modal="true" aria-label="Image viewer">
                    <button
            ...
            {showUnmatchConfirm && (
                <div className="fixed inset-0 z-[100] bg-slate-950/90 backdrop-blur-sm flex items-center justify-center p-6 animate-fade-in" role="dialog" aria-modal="true" aria-label={`Unmatch ${match.profile.name}`}>
